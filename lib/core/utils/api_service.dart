...
class ApiService {
  ApiService(this._dio);
  final _baseUrl = 'http://192.168.56.1:8888/api';
  final Dio _dio;

  Future<Map<String, dynamic>> post(
      {required dynamic data, required dynamic endPoint}) async {
    var response = await _dio.post(
      '$_baseUrl$endPoint',
      data: data,
      options: Options(headers: {
        'accept': 'application/json',
        'authorization':
            'Bearer 1|882a2OwOiSILPmr5pUIEpAmiaF7DMtmWj0524PuOa6aa03da'
      }),
    );
